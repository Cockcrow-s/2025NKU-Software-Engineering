...
class HandGestureRecognizer:
    ...
    def _recognize_gesture(self, hand_landmarks, frame_width, frame_height):
        ...
        # 握拳手势检测
        fist_gesture = self._is_fist_gesture(points)
        if fist_gesture:
            return "fist"
    ...
    def _is_fist_gesture(self, points):
        """检测握拳手势"""
        finger_tips = [points[8], points[12], points[16], points[20]]  
        finger_pip = [points[6], points[10], points[14], points[18]]  
        
        # 手腕和手掌中心
        wrist = points[0]
        palm_center = points[9]  # 中指掌指关节作为手掌中心参考
        
        # 检查所有手指是否弯曲
        fingers_bent = all(tip[1] > pip[1] for tip, pip in zip(finger_tips, finger_pip))
        
        # 检查大拇指是否弯曲
        thumb_tip = points[4]
        thumb_ip = points[3]
        thumb_bent = abs(thumb_tip[0] - thumb_ip[0]) < 30  # 大拇指水平距离较小
        
        # 检查手指是否聚拢在手掌中心附近
        fingers_close = all(
            np.sqrt((tip[0] - palm_center[0])**2 + (tip[1] - palm_center[1])**2) < 80
            for tip in finger_tips
        )
        
        return fingers_bent and thumb_bent and fingers_close
