...
class ModernCard(QFrame):
    ...
    def __init__(self, title="", content="", icon_path=""):
        ...
        if content:
            ...
            content_label.setStyleSheet("color: white; line-height: 1.4;")
...
from system_ui import SystemManagementWindow  
...
class MainWindow(QMainWindow):
    ...
    def create_right_panel(self, parent_layout):
        ...
        warning_group.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Maximum)
        ...
        warning_layout.setContentsMargins(10, 15, 10, 10)
        
        self.warning_text = QTextEdit()
        self.warning_text.setMinimumHeight(80)
        self.warning_text.setMaximumHeight(120)
        self.warning_text.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
    ...
    def init_threads(self):
        ...
        # 语音线程
        print("启动语音识别线程...")
        self.voice_thread = VoiceThread(self.music_player, self.user)  # 传入用户对象
        ...
        # 手势线程 - 传入用户信息
        print("启动手势识别线程...")
        self.gesture_thread = GestureThread(self.user)  # 传入用户对象
        ...
        # 连接音乐控制信号
        self.gesture_thread.music_control.connect(self.on_music_control)
        self.gesture_thread.start_gesture_recognition()
        print("手势识别系统已启动")
        
        # 面部识别线程 - 传入用户信息
        print("启动面部识别线程...")
        self.face_thread = FaceThread(self.user)  # 传入用户对象
    ...
    def on_music_control(self, action):
        """处理手势音乐控制"""
        try:
            if action == "play_pause":
                if self.music_player.state == "playing":
                    ...
                    print("🎵 通过手势暂停音乐")
                    self.warning_text.setPlainText("🎵 通过握拳手势暂停音乐")
                else:
                    self.music_player.play()
                    print("🎵 通过手势播放音乐")
                    self.warning_text.setPlainText("🎵 通过握拳手势开始播放音乐")
            elif action == "next":
                self.music_player.next()
                print("🎵 通过手势切换到下一首")
                self.warning_text.setPlainText("🎵 通过挥手手势切换到下一首音乐")
        except Exception as e:
            print(f"音乐控制错误: {e}")
            self.warning_text.setPlainText(f"🎵 音乐控制失败: {str(e)}")
    ...
    def on_error_occurred(self, error_msg):
        ...
        if "Camera" in error_msg:
            self.warning_text.setPlainText(f"📷 摄像头错误: {error_msg}\n请检查摄像头连接或权限设置")
        elif "Visual recognition" in error_msg:
            self.warning_text.setPlainText(f"👁️  视觉识别错误: {error_msg}\n将以基础模式运行")
        else:
            self.warning_text.setPlainText(f"⚠️  系统错误: {error_msg}")
        
        # 更新状态显示
        self.visual_status.setText("错误")
        self.visual_status.setStyleSheet("""
            QLabel {
                background-color: #DC3545;
                color: white;
    ...
    def start_camera(self):
        ...
        try:
            ...
            print("✓ 摄像头启动成功（隐私保护模式）")
        except Exception as e:
            print(f"启动摄像头时发生错误: {e}")
            self.on_error_occurred(f"启动失败: {str(e)}")
    ...
    def update_video_display(self, frame):
        """更新视频显示 - 不显示画面，仅处理帧数据"""
        # 不显示摄像头画面，仅保持连接以确保系统正常工作
        # 可以在这里添加帧计数或其他统计信息，但不显示实际画面
        pass
    
    def on_distraction_detected(self, distracted, warning_level):
        """处理分心检测 - 三模态异常反馈场景"""
        if distracted:
            print(f"检测到分心驾驶！警告级别: {warning_level}")
            ...
            # 等待驾驶员确认 - 可通过语音说"已注意道路"或手势确认
            print("等待驾驶员确认：说'已注意道路'或竖起大拇指确认")
    ...
    def update_card_content(self, card, content):
        """更新卡片内容"""
        # 查找卡片内的内容标签
        for child in card.findChildren(QLabel):
            if child.font().pointSize() == 12:  # 内容标签的字体大小
    ...
    def on_gesture_recognized(self, gesture):
        ...
        # 更新手势信息显示
        if hasattr(self, 'gesture_info'):
            new_content = f"当前手势: {gesture_text}\n上次识别: 刚刚\n状态: 已处理"
            self.update_card_content(self.gesture_info, new_content)
    ...
    def on_gesture_action(self, action):
        ...
        if action == "confirm":
            # 一般确认动作处理
            print("驾驶员执行了确认动作")
            self.warning_text.setPlainText("✅ 驾驶员执行了确认动作")
            ...
            # 一般拒绝动作处理
            print("驾驶员执行了拒绝动作")
            self.warning_text.setPlainText("❌ 驾驶员执行了拒绝动作")
            ...
            # 停止动作处理
            print("驾驶员执行了停止动作")
            self.warning_text.setPlainText("✋ 驾驶员请求停止当前操作")
        else:
            # 其他动作处理
    ...
    def closeEvent(self, event):
     ...
     # 停止gesture_thread
        ...
        if hasattr(self, 'voice_thread'):
            print("停止语音识别线程...")
            self.voice_thread.stop_thread()  # 使用新的停止方法
